...
> **Post-processing GLSL (camera) — `src/PostFx.ts` + `src/pipelines/FilmPipeline.ts`:** evoluzione AAA degli overlay "finti". Una **singola passata** a livello camera (WebGL) sostituisce vignetta/grana/scanline/aberrazione-finta di `Juice` con versioni *fisiche* in shader, più **bloom** integrato di Phaser (multi-pass a mezza risoluzione) sugli emissivi. Tutto gated da `Settings.screenFx`; su renderer Canvas (fallback `Phaser.AUTO`) si ripiega automaticamente sugli overlay di `Juice`. **Nessun asset esterno**: il GLSL è inline nel `.ts`. Catena: aberrazione radiale → tone mapping ACES → contrasto → saturazione → temperatura → vignetta → grana animata → scanline.
> **Parametri di default (🎚️ regolabili a runtime via `filmParams`, fonte di verità di questo look):** `intensity 1` · `contrast 1.08` · `saturation 0.92` (desaturato notturno) · `warmth 0.0` (neutro; il viraggio caldo/freddo per-ambiente resta il MULTIPLY a depth 16) · `tone 0.5` + `exposure 1.05` (ACES) · `aberration 0.006` (tenue, radiale) · `grain 0.045` (≤ ~0.05) · `scanline 0.035`. Bloom: `strength 0.5`, `blur 1.1`, `steps 6`. **Cinetica (M5, readability-safe):** streak radiale **solo ai bordi** durante scatto (`speed→1`) e Sovraccarico (`speed→0.5`) — il centro resta nitido; più uno **shock** transitorio di aberrazione su scatto/overdrive (decay 0.88/frame). Motion-blur e DoF a tutto schermo sono **esclusi** di proposito (danneggiano la leggibilità, contro l'art bible). **Nota:** il post-FX di camera tocca anche l'HUD → default conservativi per la leggibilità; isolare l'HUD su una camera dedicata è un'evoluzione possibile se necessario.
> **Ombre di contatto (radicamento 2.5D) — `src/Shadows.ts`:** ✅ ogni entità viva (veicolo, zombi, boss, taniche, relitti/mine) proietta un'**ellisse morbida a terra**, ridisegnata per frame in un'unica RenderTexture (depth 4, sotto le entità). È l'interpretazione *art-compatibile* della "luce 2.5D": gli sprite sono già pre-ombreggiati (luce alto-sinistra cotta), quindi il normal-mapping per-sprite darebbe doppia ombra; la drop shadow invece radica le entità nel mondo (look Darkest Dungeon / Don't Starve) soddisfacendo il pilastro 7 ("ombre a terra coerenti su tutto"). Taratura: larghezza 0.92×sprite, alpha 0.34, scostamento basso-destra (coerente con la luce). L'olio piatto non proietta ombra. **Due toni** (M4): penombra morbida larga + **core AO** stretto e denso al punto di contatto (radica davvero l'oggetto); veicolo e boss pesano di più (alpha ×1.25). I **decal** (sangue su uccisioni, skid, scorch, debris) erano già completi in `Environment` e scorrono con la strada.
...
- **Performance budget:** decine di nemici a schermo a 60 fps. Niente effetti per-pixel sulle entità, niente blur sugli sprite. Il post-processing a livello camera (FilmPipeline) è una singola passata leggera + bloom a mezza risoluzione (vedi §"Standard di Produzione AAA").
