...
> **Parametri di default (🎚️ regolabili a runtime via `filmParams`, fonte di verità di questo look):** `intensity 1` · `contrast 1.08` · `saturation 0.92` (desaturato notturno) · `warmth 0.0` (neutro; il viraggio caldo/freddo per-ambiente resta il MULTIPLY a depth 16) · `tone 0.5` + `exposure 1.05` (ACES) · `aberration 0.006` (tenue, radiale) · `grain 0.045` (≤ ~0.05) · `scanline 0.035`. Bloom: `strength 0.5`, `blur 1.1`, `steps 6`. **Nota:** il post-FX di camera tocca anche l'HUD → default conservativi per la leggibilità; isolare l'HUD su una camera dedicata è un'evoluzione possibile se necessario.
