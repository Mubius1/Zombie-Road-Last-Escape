...
const ZOMBIE_INFO: ZombieInfo[] = [
  ...
  { key: 'zombie_runner',  label: 'Corridore' },
  { key: 'zombie_armored', label: 'Corazzato' },
  { key: 'zombie_jumper',  label: 'Saltatore' },
  { key: 'zombie_toxic',   label: 'Tossico' },
  { key: 'zombie_giant',   label: 'Gigante' },
];

const OBJECT_INFO: Array<{ key: string; label: string; scale: number }> = [
...
export default class DebugScene extends Phaser.Scene {
  ...
  private sectionTitle(x: number, y: number, text: string, color = '#88aaff') {
  ...
  private drawWeapons(y: number) {
    this.sectionTitle(12, y, 'ARMI  (clicca = prova)', '#ff9944');
    const top = y + 16;
  ...
  private drawButtons(y: number) {
    ...
    // Riga 1: setup test
    mk(90,  150, '★ +5000 monete', 0x665500, () => this.toast('+5000 monete', () =>
      this.registry.set('money', (this.registry.get('money') ?? 0) + 5000)));
    mk(250, 150, '🔫 Tutte le armi', 0x664400, () => this.toast('Armi sbloccate', () => {
      this.registry.set('ownedWeapons', [...WEAPON_KEYS]);
    }));
    mk(410, 150, '🚗 Tutti i veicoli', 0x445566, () => this.toast('Veicoli sbloccati', () => {
      this.registry.set('ownedVehicles', [...VEHICLE_KEYS]);
    }));
    mk(570, 150, '👥 Recluta tutti', 0x556644, () => this.toast('Sopravvissuti reclutati', () => {
