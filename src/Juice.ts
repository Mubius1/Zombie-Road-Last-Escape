...
export default class Juice {
  ...
  static buildTextures(scene: Phaser.Scene) {
    if (scene.textures.exists('fx_grain')) return;
    ...
    const R = 32;
    const l = scene.make.graphics({ add: false } as any) as Phaser.GameObjects.Graphics & { generateTexture(k: string, w: number, h: number): void };
    for (let i = R; i >= 1; i--) {
    ...
    const s = scene.make.graphics({ add: false } as any) as Phaser.GameObjects.Graphics & { generateTexture(k: string, w: number, h: number): void };
    s.fillStyle(0x000000, 1); s.fillRect(0, 0, 4, 1);
  ...
  static addOverlay(scene: Phaser.Scene, depth = 18): Phaser.GameObjects.TileSprite {
    ...
    fr.fillGradientStyle(0xff0022, 0xff0022, 0xff0022, 0xff0022, 0.07, 0, 0.07, 0); fr.fillRect(0, 0, 48, H);
    fr.fillGradientStyle(0x00ddff, 0x00ddff, 0x00ddff, 0x00ddff, 0, 0.07, 0, 0.07); fr.fillRect(W - 48, 0, 48, H);
  ...
  static jitterGrain(grain: Phaser.GameObjects.TileSprite | null) {
    if (!grain) return;
  ...
  static flash(scene: Phaser.Scene, color = 0xffffff, alpha = 0.4, ms = 120) {
    const W = scene.scale.width, H = scene.scale.height;
  ...
  static muzzleFlash(scene: Phaser.Scene, x: number, y: number, color = 0xffdd66) {
  ...
  static bloomBurst(scene: Phaser.Scene, x: number, y: number, color: number, size = 2.5, ms = 260) {
   ...
   */
  static lightFlash(scene: Phaser.Scene, x: number, y: number, color: number, scale = 5, ms = 320) {
  ...
  static fadeIn(scene: Phaser.Scene, ms = 300) {
    scene.cameras.main.fadeIn(ms, 0, 0, 0);
  }

  /** Sfuma a nero, poi esegue il callback. */
  static fadeAndRun(scene: Phaser.Scene, cb: () => void, ms = 320) {
  ...
  static go(scene: Phaser.Scene, key: string, ms = 320) {
