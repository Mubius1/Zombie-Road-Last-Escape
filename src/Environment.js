...
export default class Environment {
    constructor(scene, geom, env, envIndex) {
    ...
    gfx() {
        return this.scene.make.graphics({ add: false });
    }
    ...
    buildAsphaltTexture() {
        ...
        for (let i = 0; i < 2; i++) {
            ...
            g.fillStyle(this.patchColor, 0.9);
            g.fillRect(px, py, pw, ph);
            g.lineStyle(2, Environment.mix(this.patchColor, 0x000000, 0.4), 0.8);
        ...
        for (let i = 0; i < 7; i++) {
            ...
            for (let s = 0; s < segs; s++) {
                ...
                if (s === 1 && Math.random() < 0.6)
                    g.lineBetween(x, y, x + (Math.random() * 24 - 12), y + (Math.random() * 20 - 10));
                x = nx;
                y = ny;
            }
        }
    ...
    buildParallaxTextures() {
        ...
        if (!this.has(farKey)) {
            const g = this.gfx();
            this.drawFar(g, FH);
            g.generateTexture(farKey, PW, FH);
            g.destroy();
        }
        if (!this.has(nearKey)) {
            const g = this.gfx();
            this.drawNear(g, NH);
            g.generateTexture(nearKey, PW, NH);
            g.destroy();
        }
    }
    ...
    drawFar(g, FH) {
        ...
        const lit = (i, j) => ((i * 7 + j * 13 + this.idx) % 3) === 0; // finestre deterministiche
        switch (this.idx) {
                ...
                for (let i = 0; i < 6; i++) {
                    ...
                    for (let wy = B - bh + 6, j = 0; wy < B - 6; wy += 12, j++)
                        for (let wx = bx + 5, k = 0; wx < bx + bw - 5; wx += 12, k++) {
                            g.fillStyle(lit(j, k) ? 0x2a2a16 : 0x0c0c1c);
            ...
            case 1: { // Autostrada — guardrail + alberi morti
                ...
                for (let i = 0; i < 3; i++) {
                    ...
                    g.fillRect(tx - 14, B - th + 8, 12, 4);
                    g.fillRect(tx + 5, B - th + 16, 13, 4);
                }
                g.fillStyle(0x3a3830);
                g.fillRect(0, B - 14, PW, 4);
                for (let x = 0; x < PW; x += 40)
                    g.fillRect(x, B - 20, 4, 12);
                break;
            }
            case 2: { // Deserto — dune + cactus
                ...
                for (let i = 0; i < 3; i++) {
                    ...
                    g.fillRect(x - 8, B - 38, 12, 8);
                    g.fillRect(x - 8, B - 50, 8, 14);
                    g.fillRect(x + 14, B - 33, 12, 8);
                    g.fillRect(x + 20, B - 45, 8, 14);
                }
                break;
            }
            case 3: { // Foresta — pini a triangolo (periodo 40)
                ...
                for (let i = 0; i < 12; i++) {
                    ...
                    for (let dy = 0; dy < th; dy++) {
                        const hw = Math.round((dy / th) * 17);
                        g.fillRect(cx - hw, B - th + dy, hw * 2, 3);
                    }
            ...
            case 4: { // Zona Industriale — fabbrica + ciminiere
                ...
                g.fillRect(0, B - 34, PW, 34);
                g.fillStyle(0x2a2420);
                for (let i = 0; i < 4; i++) {
                    const sx = i * 120 + 40, sh = 64 + (i % 3) * 18;
                    g.fillRect(sx, B - sh, 20, sh);
                    g.fillRect(sx - 4, B - sh, 28, 8);
                    g.fillStyle(0x181614);
                    g.fillCircle(sx + 10, B - sh - 8, 9);
                    g.fillStyle(0x2a2420);
                }
                break;
            }
            case 5: { // Base Militare — recinzione + torrette
                ...
                for (let i = 0; i < 2; i++) {
                    const tx = i * 240 + 90;
                    g.fillRect(tx + 4, B - 74, 7, 74);
                    g.fillRect(tx - 18, B - 80, 46, 18);
                    g.fillRect(tx - 20, B - 86, 50, 8);
                    g.fillStyle(0x446644);
                    g.fillRect(tx - 6, B - 74, 5, 10);
                    g.fillStyle(0x1e2a14);
                }
                g.fillStyle(0x2a3820);
                g.fillRect(0, B - 18, PW, 4);
                for (let x = 0; x < PW; x += 16)
                    g.fillRect(x, B - 26, 3, 12);
                break;
            }
                ...
                for (let i = 0; i < 6; i++) {
                    ...
                    for (let wy = B - bh + 6, j = 0; wy < B - 6; wy += 11, j++)
                        for (let wx = bx + 5, k = 0; wx < bx + bw - 5; wx += 10, k++) {
                            g.fillStyle(lit(j, k) ? 0x4a1a6a : 0x0e060e);
    ...
    drawNear(g, _NH) {
        switch (this.idx) {
            case 0:
                ...
                for (let x = 0; x < PW; x += 96)
                    g.fillRect(x + 10, 4, 38, 14);
                break;
            case 1:
                g.fillStyle(0x3a3830);
                g.fillRect(0, 2, PW, 4);
                g.fillRect(0, 14, PW, 3);
                for (let x = 0; x < PW; x += 40)
                    g.fillRect(x, 0, 4, 18);
                break;
            case 2:
                ...
                for (let x = 0; x <= PW; x += 2) {
                    const h = Math.round(Math.sin(x / PW * Math.PI * 2 * 4) * 12 + 8);
                    g.fillRect(x, 0, 2, h);
                }
                break;
            case 3:
                ...
                for (let x = 0; x < PW; x += 48)
                    g.fillRect(x, 0, 32, 8 + (x % 4) * 3);
                break;
            case 4:
                g.fillStyle(0x302820);
                g.fillRect(0, 4, PW, 10);
                g.fillRect(0, 20, PW, 6);
                for (let x = 0; x < PW; x += 80)
                    g.fillRect(x, 0, 14, 28);
                break;
            case 5:
                g.fillStyle(0x2a2a1a);
                for (let x = 0; x < PW; x += 48) {
                    g.fillRect(x, 2, 42, 14);
                    g.fillRect(x + 5, 0, 32, 10);
                }
                break;
            default:
                g.fillStyle(0x160820);
                g.fillRect(0, 0, PW, _NH);
                g.fillStyle(0x220c30);
                for (let x = 0; x < PW; x += 120)
                    g.fillRect(x, 0, 50, 40);
                g.fillStyle(this.emissive, 0.10);
                for (let x = 30; x < PW; x += 120)
                    g.fillEllipse(x, 18, 60, 14);
        }
    }
    ...
    buildDecalTextures() {
        if (!this.has('env_scorch')) {
            ...
            g.fillStyle(0x3a1d0e, 0.3);
            g.fillCircle(32, 32, 16);
            g.fillStyle(0x000000, 0.4);
            g.fillCircle(32, 32, 8);
            g.generateTexture('env_scorch', 64, 64);
            g.destroy();
        }
        if (!this.has('env_blood')) {
            const g = this.gfx();
            g.fillStyle(0x4a0e0a, 0.9);
            g.fillEllipse(28, 28, 42, 30);
            g.fillStyle(0x6e1810, 0.7);
            g.fillEllipse(26, 27, 26, 18);
            g.fillStyle(0x2a0606, 0.8);
            for (let i = 0; i < 7; i++) {
                const a = Math.random() * Math.PI * 2, d = 16 + Math.random() * 12;
                g.fillEllipse(28 + Math.cos(a) * d, 28 + Math.sin(a) * d, 4 + Math.random() * 5, 3 + Math.random() * 4);
            }
            g.generateTexture('env_blood', 56, 56);
            g.destroy();
        }
        if (!this.has('env_skid')) {
            ...
            g.fillRect(0, 1, 80, 2);
            g.fillRect(0, 6, 80, 2);
            g.generateTexture('env_skid', 80, 9);
            g.destroy();
        }
        if (!this.has('env_debris')) {
            ...
            for (let i = 0; i < 9; i++)
                g.fillRect(Math.random() * 38, Math.random() * 22, 2 + Math.random() * 3, 2 + Math.random() * 3);
            g.generateTexture('env_debris', 40, 24);
            g.destroy();
        }
    }
    // ─── §6 Illuminazione: cono fari ────────────────────────────────────────────
    buildHeadlightTexture() {
        if (this.has('env_headlight'))
            return;
        ...
        for (let x = 0; x < CW; x += 2) {
            ...
            g.fillEllipse(x, CH / 2, 6, halfH * 2);
        }
        g.generateTexture('env_headlight', CW, CH);
        g.destroy();
    }
    ...
    buildObjects() {
        ...
        sky.fillGradientStyle(top, top, hor, hor, 1, 1, 1, 1);
        sky.fillRect(0, 0, W, roadTop);
        sky.fillStyle(this.hazeColor, 0.22);
        sky.fillRect(0, roadTop - 16, W, 16);
        ...
        rl.fillGradientStyle(0, 0, 0, 0, 0.45, 0.45, 0, 0);
        rl.fillRect(0, roadTop, W, roadCenter - roadTop);
        rl.fillGradientStyle(0, 0, 0, 0, 0, 0, 0.5, 0.5);
        rl.fillRect(0, roadCenter, W, roadBottom - roadCenter);
    ...
    addDecal(type, x, y) {
        if (y < this.g.roadTop + 8 || y > this.g.roadBottom - 8)
            return; // solo sulla carreggiata
        let key = 'env_scorch', life = 4000, alpha = 0.9, scale = 1, rot = Math.random() * Math.PI;
        switch (type) {
            case 'blood':
                if (this.scene.time.now - this.lastBlood < BLOOD_THROTTLE)
                    return;
